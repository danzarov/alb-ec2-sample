...
### security group used by the ec2 instances spinned up by the ASG
...
resource "aws_autoscaling_group" "asg-sample" {
  ...
  target_group_arns   = [aws_lb_target_group.target_group_sample.arn] # all ec2 instances spinned up by this asg will be associated with the target group
...
resource "aws_lb_listener" "alb_listener" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.target_group_sample.arn # listener will forward traffic to the target group which is binded to the asg already
  }
}
